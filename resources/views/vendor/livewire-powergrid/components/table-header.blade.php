<tr class="{{ $theme->table->trBodyClass }}" style="{{ $theme->table->trBodyStyle }}">
    ...
    <td class="{{ $theme->table->tdBodyClassTotalColumns . ' '.$column->bodyClass ?? '' }}"
        ...
        <br>
        @endif
        @if ($column->avg['header'] && is_numeric($withoutPaginatedData[0][$field]))<span>{{ $column->avg['label'] }}:
    ...
    <th class="{{ $theme->table->thClass .' '. $column->headerClass }}" scope="col" style="{{ $theme->table->thStyle }}"
